...
const SiteFooter = (props: SiteFooterProps) => {
  const { heading, links_data, logo_data, name, year } = props;

  return (
    <footer>
      <FooterHeading heading={heading} />
      <FooterSection data={logo_data} type="logos" />
      <FooterSection data={links_data} type="links" />
      <FooterCopyRight name={name} year={year} />
    </footer>
  );
};
